import type {
  ActionConfig,
  AppPageConfig,
  DataPageConfig,
  FilterField,
  NavigationSection,
  TableColumn,
} from "../types";

const searchFilter: FilterField = {
  key: "searchTerm",
  label: "Search",
  placeholder: "Search by keyword",
  type: "text",
};

const dateRangeFilters: FilterField[] = [
  {
    key: "fromDate",
    label: "From",
    placeholder: "Start date",
    type: "date",
  },
  {
    key: "toDate",
    label: "To",
    placeholder: "End date",
    type: "date",
  },
];

const accountColumns: TableColumn[] = [
  { key: "customerId", label: "Customer Id" },
  { key: "customerName", label: "Customer Name" },
  { key: "meterId", label: "Meter Id" },
  { key: "meterType", label: "Meter Type" },
  { key: "communicationWay", label: "Communication Way" },
  { key: "tariffId", label: "Tariff Id" },
  { key: "protocolVersion", label: "Protocol Version" },
  { key: "remark", label: "Remark" },
  { key: "createTime", label: "Create Time" },
  { key: "stationId", label: "Station Id" },
];

const tokenRecordColumns: TableColumn[] = [
  { key: "receiptId", label: "Receipt Id" },
  { key: "customerId", label: "Customer Id" },
  { key: "customerName", label: "Customer Name" },
  { key: "meterId", label: "Meter Id" },
  { key: "meterType", label: "Meter Type" },
  { key: "tariffId", label: "Tariff Id" },
  { key: "vatCharge", label: "VAT Charge", align: "end" },
  { key: "totalUnit", label: "Total Unit", align: "end" },
  { key: "totalPaid", label: "Total Paid", align: "end" },
  { key: "tokenRecharge", label: "Token" },
  { key: "vend", label: "Vend" },
  { key: "time", label: "Time" },
  { key: "remark", label: "Remark" },
  { key: "stationId", label: "Station Id" },
];

const remoteColumns: TableColumn[] = [
  { key: "status", label: "Status" },
  { key: "customerName", label: "Customer Name" },
  { key: "meterId", label: "Meter Id" },
  { key: "meterType", label: "Meter Type" },
  { key: "remark", label: "Remark" },
  { key: "stationId", label: "Station Id" },
];

const taskColumns: TableColumn[] = [
  { key: "status", label: "Status" },
  { key: "meterId", label: "Meter Id" },
  { key: "customerName", label: "Customer Name" },
  { key: "createTime", label: "Create Time" },
  { key: "remark", label: "Remark" },
  { key: "stationId", label: "Station Id" },
];

const intervalColumns: TableColumn[] = [
  { key: "meterId", label: "Meter Id" },
  { key: "gatewayId", label: "Gateway Id" },
  { key: "collectionDate", label: "Collection Date" },
  { key: "customerId", label: "Customer Id" },
  { key: "customerName", label: "Customer Name" },
  { key: "stationId", label: "Station Id" },
  { key: "totalEnergy", label: "Total Energy", align: "end" },
  { key: "lastHourUsage", label: "Last Hour Usage", align: "end" },
  { key: "creditBalance", label: "Credit Balance", align: "end" },
  { key: "maximumDemand", label: "Maximum Demand", align: "end" },
  { key: "protocol", label: "Protocol" },
];

const customerColumns: TableColumn[] = [
  { key: "id", label: "Id" },
  { key: "name", label: "Name" },
  { key: "phone", label: "Phone" },
  { key: "address", label: "Address" },
  { key: "certifiName", label: "Certifi Name" },
  { key: "certifiNo", label: "Certifi No" },
  { key: "remark", label: "Remark" },
  { key: "createTime", label: "Create Time" },
  { key: "updateTime", label: "Update Time" },
  { key: "stationId", label: "Station Id" },
];

const simpleCrudColumns: TableColumn[] = [
  { key: "id", label: "Id" },
  { key: "name", label: "Name" },
  { key: "remark", label: "Remark" },
  { key: "createTime", label: "Create Time" },
  { key: "updateTime", label: "Update Time" },
  { key: "stationId", label: "Station Id" },
];

function createRechargeAction(label: string, endpoint: string): ActionConfig {
  return {
    key: "generate",
    label,
    endpoint,
    tone: "primary",
    operationKind: "token-generate",
    fields: [
      {
        key: "amount",
        label: "Amount",
        type: "number",
        placeholder: "Enter amount",
      },
      {
        key: "unit",
        label: "Unit",
        type: "number",
        placeholder: "Enter unit",
      },
    ],
  };
}

function createTokenGeneratePage(
  path: string,
  title: string,
  menuLabel: string,
  endpoint: string,
): DataPageConfig {
  return {
    kind: "data",
    path,
    title,
    menuLabel,
    description: `${title} with account search, pagination, and action modal.`,
    sectionKey: "token-generate",
    readEndpoint: "/api/account/read",
    readOperationKind: "table-read",
    filters: [searchFilter],
    columns: accountColumns,
    rowActions: [createRechargeAction(menuLabel, endpoint)],
  };
}

function createTokenRecordPage(
  path: string,
  title: string,
  menuLabel: string,
  readEndpoint: string,
): DataPageConfig {
  return {
    kind: "data",
    path,
    title,
    menuLabel,
    description: `${title} with export, print, and cancellation actions.`,
    sectionKey: "token-record",
    readEndpoint,
    readOperationKind: "table-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: tokenRecordColumns,
    toolbarActions: [
      {
        key: "export",
        label: "Export",
        endpoint: `${readEndpoint.replace("/read", "/export")}`,
        tone: "neutral",
        operationKind: "report-export",
      },
    ],
    rowActions: [
      {
        key: "print",
        label: "Print",
        endpoint: `${readEndpoint.replace("/read", "/print")}`,
        tone: "neutral",
        operationKind: "generic",
      },
      {
        key: "cancel",
        label: "Cancel",
        endpoint: `${readEndpoint.replace("/read", "/cancel")}`,
        tone: "danger",
        operationKind: "record-cancel",
        confirmMessage: "Cancel this record?",
      },
    ],
  };
}

function createRemoteOperationPage(
  path: string,
  title: string,
  menuLabel: string,
  endpoint: string,
): DataPageConfig {
  return {
    kind: "data",
    path,
    title,
    menuLabel,
    description: `${title} with meter search and task creation flow.`,
    sectionKey: "remote-operation",
    readEndpoint: "/api/meter/read",
    readOperationKind: "table-read",
    filters: [searchFilter],
    columns: remoteColumns,
    toolbarActions: [
      {
        key: "batch",
        label: "Add Batch Task",
        endpoint,
        tone: "primary",
        operationKind: "task-create",
        fields: [
          {
            key: "taskName",
            label: "Task Name",
            placeholder: "Task name",
          },
          {
            key: "scheduleDate",
            label: "Schedule Date",
            placeholder: "Schedule date",
            type: "date",
          },
        ],
      },
    ],
    rowActions: [
      {
        key: "single",
        label: "Add Task",
        endpoint,
        tone: "primary",
        operationKind: "task-create",
        fields: [
          {
            key: "taskName",
            label: "Task Name",
            placeholder: "Task name",
          },
        ],
      },
    ],
  };
}

function createTaskPage(
  path: string,
  title: string,
  menuLabel: string,
  readEndpoint: string,
  updateEndpoint: string,
): DataPageConfig {
  return {
    kind: "data",
    path,
    title,
    menuLabel,
    description: `${title} for monitoring and updating queued operations.`,
    sectionKey: "remote-operation-task",
    readEndpoint,
    readOperationKind: "task-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: taskColumns,
    toolbarActions: [
      {
        key: "refresh",
        label: "Refresh Tasks",
        endpoint: readEndpoint,
        tone: "neutral",
        operationKind: "generic",
      },
    ],
    rowActions: [
      {
        key: "update",
        label: "Update Task",
        endpoint: updateEndpoint,
        tone: "primary",
        operationKind: "task-update",
        fields: [
          {
            key: "remark",
            label: "Remark",
            placeholder: "Update remark",
          },
        ],
      },
    ],
  };
}

function createManagementPage(
  path: string,
  title: string,
  menuLabel: string,
  readEndpoint: string,
  columns: TableColumn[],
): DataPageConfig {
  const base = readEndpoint.replace("/read", "");

  return {
    kind: "data",
    path,
    title,
    menuLabel,
    description: `${title} with standard CRUD, import, export, and bulk actions.`,
    sectionKey: "management",
    readEndpoint,
    readOperationKind: "table-read",
    filters: [searchFilter],
    columns,
    toolbarActions: [
      {
        key: "add",
        label: "Add",
        endpoint: `${base}/create`,
        tone: "primary",
        operationKind: "management-create",
        fields: [
          { key: "name", label: "Name", placeholder: `${menuLabel} name` },
          { key: "remark", label: "Remark", placeholder: "Optional note" },
        ],
      },
      {
        key: "import",
        label: "Import",
        endpoint: `${base}/import`,
        tone: "neutral",
        operationKind: "generic",
        fields: [
          { key: "source", label: "Source", placeholder: "File reference" },
        ],
      },
      {
        key: "export",
        label: "Export",
        endpoint: `${base}/export`,
        tone: "neutral",
        operationKind: "report-export",
      },
    ],
    bulkActions: [
      {
        key: "bulk-delete",
        label: "Delete Selected",
        endpoint: `${base}/delete`,
        tone: "danger",
        operationKind: "bulk-delete",
        confirmMessage: "Delete the selected records?",
      },
    ],
    rowActions: [
      {
        key: "edit",
        label: "Edit",
        endpoint: `${base}/update`,
        tone: "primary",
        operationKind: "management-update",
        fields: [
          { key: "name", label: "Name", placeholder: `${menuLabel} name` },
          { key: "remark", label: "Remark", placeholder: "Optional note" },
        ],
      },
      {
        key: "delete",
        label: "Delete",
        endpoint: `${base}/delete`,
        tone: "danger",
        operationKind: "management-delete",
        confirmMessage: "Delete this record?",
      },
    ],
  };
}

const pages: AppPageConfig[] = [
  {
    kind: "dashboard",
    path: "/dashboard",
    title: "Dashboard",
    menuLabel: "Dashboard",
    description: "Overview of KPIs, purchase patterns, alarms, and consumption.",
    sectionKey: "dashboard",
  },
  createTokenGeneratePage(
    "/token-generate/credit-token",
    "Credit Token",
    "Credit Token",
    "/api/token/creditToken/generate",
  ),
  createTokenGeneratePage(
    "/token-generate/clear-tamper-token",
    "Clear Tamper Token",
    "Clear Tamper Token",
    "/api/token/clearTamperToken/generate",
  ),
  createTokenGeneratePage(
    "/token-generate/clear-credit-token",
    "Clear Credit Token",
    "Clear Credit Token",
    "/api/token/clearCreditToken/generate",
  ),
  createTokenGeneratePage(
    "/token-generate/set-max-power-limit-token",
    "Set Maximum Power Limit Token",
    "Set Max Power Limit",
    "/api/token/setMaxPowerLimitToken/generate",
  ),
  createTokenRecordPage(
    "/token-record/credit-token-record",
    "Credit Token Record",
    "Credit Token Record",
    "/api/token/creditTokenRecord/read",
  ),
  createTokenRecordPage(
    "/token-record/clear-tamper-record",
    "Clear Tamper Record",
    "Clear Tamper Record",
    "/api/token/clearTamperTokenRecord/read",
  ),
  createTokenRecordPage(
    "/token-record/clear-credit-record",
    "Clear Credit Record",
    "Clear Credit Record",
    "/api/token/clearCreditTokenRecord/read",
  ),
  createTokenRecordPage(
    "/token-record/set-max-power-limit-record",
    "Set Maximum Power Limit Record",
    "Set Max Power Record",
    "/api/token/setMaxPowerLimitTokenRecord/read",
  ),
  createRemoteOperationPage(
    "/remote-operation/meter-reading",
    "Meter Reading",
    "Meter Reading",
    "/API/RemoteMeterTask/CreateReadingTask",
  ),
  createRemoteOperationPage(
    "/remote-operation/meter-control",
    "Meter Control",
    "Meter Control",
    "/API/RemoteMeterTask/CreateControlTask",
  ),
  createRemoteOperationPage(
    "/remote-operation/meter-token",
    "Meter Token",
    "Meter Token",
    "/API/RemoteMeterTask/CreateTokenTask",
  ),
  createTaskPage(
    "/remote-operation-task/meter-reading-task",
    "Meter Reading Task",
    "Meter Reading Task",
    "/API/RemoteMeterTask/GetReadingTask",
    "/API/RemoteMeterTask/UpdateReadingTask",
  ),
  createTaskPage(
    "/remote-operation-task/meter-control-task",
    "Meter Control Task",
    "Meter Control Task",
    "/API/RemoteMeterTask/GetControlTask",
    "/API/RemoteMeterTask/UpdateControlTask",
  ),
  createTaskPage(
    "/remote-operation-task/meter-token-task",
    "Meter Token Task",
    "Meter Token Task",
    "/API/RemoteMeterTask/GetTokenTask",
    "/API/RemoteMeterTask/UpdateTokenTask",
  ),
  {
    kind: "data",
    path: "/data-report/long-nonpurchase",
    title: "Long Nonpurchase Situation",
    menuLabel: "Long Nonpurchase",
    description: "Report view for long nonpurchase trends with date filters.",
    sectionKey: "data-report",
    readEndpoint: "/API/PrepayReport/LongNonpurchaseSituation",
    readOperationKind: "report-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: [
      { key: "customerId", label: "Customer Id" },
      { key: "customerName", label: "Customer Name" },
      { key: "meterId", label: "Meter Id" },
      { key: "lastPurchaseDate", label: "Last Purchase Date" },
      { key: "daysWithoutPurchase", label: "Days Without Purchase", align: "end" },
      { key: "stationId", label: "Station Id" },
    ],
    toolbarActions: [
      {
        key: "export",
        label: "Export",
        endpoint: "/API/PrepayReport/LongNonpurchaseSituationExport",
        tone: "neutral",
        operationKind: "report-export",
      },
    ],
  },
  {
    kind: "data",
    path: "/data-report/low-purchase",
    title: "Low Purchase Situation",
    menuLabel: "Low Purchase",
    description: "Report view for low purchase accounts and balances.",
    sectionKey: "data-report",
    readEndpoint: "/API/PrepayReport/LowPurchaseSituation",
    readOperationKind: "report-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: [
      { key: "customerId", label: "Customer Id" },
      { key: "customerName", label: "Customer Name" },
      { key: "meterId", label: "Meter Id" },
      { key: "remainingBalance", label: "Remaining Balance", align: "end" },
      { key: "lastPurchaseDate", label: "Last Purchase Date" },
      { key: "stationId", label: "Station Id" },
    ],
    toolbarActions: [
      {
        key: "export",
        label: "Export",
        endpoint: "/API/PrepayReport/LowPurchaseSituationExport",
        tone: "neutral",
        operationKind: "report-export",
      },
    ],
  },
  {
    kind: "data",
    path: "/data-report/consumption-statistics",
    title: "Consumption Statistics",
    menuLabel: "Consumption Statistics",
    description: "Report view for aggregated consumption performance.",
    sectionKey: "data-report",
    readEndpoint: "/API/PrepayReport/ConsumptionStatistics",
    readOperationKind: "report-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: [
      { key: "meterId", label: "Meter Id" },
      { key: "customerName", label: "Customer Name" },
      { key: "periodStart", label: "Period Start" },
      { key: "periodEnd", label: "Period End" },
      { key: "totalEnergy", label: "Total Energy", align: "end" },
      { key: "stationId", label: "Station Id" },
    ],
    toolbarActions: [
      {
        key: "export",
        label: "Export",
        endpoint: "/API/PrepayReport/ConsumptionStatisticsExport",
        tone: "neutral",
        operationKind: "report-export",
      },
    ],
  },
  {
    kind: "data",
    path: "/data-report/interval-data",
    title: "Interval Data",
    menuLabel: "Interval Data",
    description: "Interval report with meter drill-down action for hourly views.",
    sectionKey: "data-report",
    readEndpoint: "/api/DailyDataMeter/read",
    readOperationKind: "report-read",
    filters: [searchFilter, ...dateRangeFilters],
    columns: intervalColumns,
    rowActions: [
      {
        key: "hourly",
        label: "Hourly",
        endpoint: "/api/DailyDataMeter/readHourly",
        tone: "primary",
        operationKind: "generic",
      },
    ],
    toolbarActions: [
      {
        key: "monthly",
        label: "Monthly View",
        endpoint: "/api/DailyDataMeter/readMonthly",
        tone: "neutral",
        operationKind: "generic",
      },
    ],
  },
  createManagementPage(
    "/management/customer",
    "Customer Management",
    "Customer",
    "/api/customer/read",
    customerColumns,
  ),
  createManagementPage(
    "/management/account",
    "Account Management",
    "Account",
    "/api/account/read",
    accountColumns,
  ),
  createManagementPage(
    "/management/tariff",
    "Tariff Management",
    "Tariff",
    "/api/tariff/read",
    simpleCrudColumns,
  ),
  createManagementPage(
    "/management/gateway",
    "Gateway Management",
    "Gateway",
    "/api/gateway/read",
    simpleCrudColumns,
  ),
];

const sectionDefinitions = [
  { key: "dashboard", label: "Dashboard" },
  { key: "token-generate", label: "Token Generate" },
  { key: "token-record", label: "Token Record" },
  { key: "remote-operation", label: "Remote Operation" },
  { key: "remote-operation-task", label: "Remote Operation Task" },
  { key: "data-report", label: "Data Report" },
  { key: "management", label: "Management" },
];

export const defaultPath = "/dashboard";
export const allPages = pages;
export const pagesByPath = Object.fromEntries(
  pages.map((page) => [page.path, page]),
) as Record<string, AppPageConfig>;

export const navigationSections: NavigationSection[] = sectionDefinitions.map(
  (section): NavigationSection => ({
    key: section.key,
    label: section.label,
    items: pages.filter((page) => page.sectionKey === section.key),
  }),
);
