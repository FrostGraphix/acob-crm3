...
function forceReferenceHash() {
  if (window.location.hash !== REFERENCE_HASH) {
    window.location.hash = REFERENCE_HASH;
  }
}

function bootReferenceCreditTokenRecordApp() {
  if (!window.__referenceCreditTokenRecordBootPromise) {
    window.__referenceCreditTokenRecordBootPromise = (async () => {
      configureReferenceGlobals();
      ensureReferenceStylesheet("/static/css/chunk-libs.f22c819a.css");
      ensureReferenceStylesheet("/static/css/app.e3f005e0.css");
      await loadReferenceScript("/static/js/chunk-elementUI.5ead3f0c.js");
      await loadReferenceScript("/static/js/chunk-libs.e467f7d2.js");
      ensureReferenceInlineScript("reference-record-guard-bypass", REFERENCE_GUARD_BYPASS_SCRIPT);
      ensureReferenceInlineScript("reference-record-runtime", REFERENCE_RUNTIME_SCRIPT);
      await loadReferenceScript("/static/js/app.11575024.js");
      window.__referenceCreditTokenRecordAssetsLoaded = true;
    })();
  }

  return window.__referenceCreditTokenRecordBootPromise;
}

export function ReferenceCreditTokenRecordPage() {
  useEffect(() => {
    document.title = "Credit Token Record";
    forceReferenceHash();

    const hashGuard = window.setInterval(() => {
      forceReferenceHash();
    }, 150);

    const stopHashReset = window.setTimeout(() => {
      window.clearInterval(hashGuard);
    }, 12000);
    ...
    void bootReferenceCreditTokenRecordApp().then(() => {
      forceReferenceHash();
      window.setTimeout(() => {
        forceReferenceHash();
      }, 0);
    ...
    return () => {
      window.clearInterval(hashGuard);
      window.clearTimeout(stopHashReset);
...
export default ReferenceCreditTokenRecordPage;
